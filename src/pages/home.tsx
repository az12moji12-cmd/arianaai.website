 ...
 * document ("light at the end of the tunnel"). Built from staggered CSS
 * animations rather than JS/canvas so it stays light and battery-friendly,
 * and it is fully disabled for users who prefer reduced motion.
 */
function TunnelAperture() {
  ...
  return (
    <div className="tunnel-aperture" aria-hidden="true">
      <style>{`
        .tunnel-aperture {
          position: absolute;
          inset: 0;
        ...
        .tunnel-aperture .field {
          position: absolute;
          inset: 0;
          background:
            radial-gradient(circle at 50% 42%, rgba(56,189,248,0.16) 0%, rgba(56,189,248,0) 55%),
            linear-gradient(180deg, #050b18 0%, #0a1a33 45%, #0f2447 100%);
        }
        .tunnel-aperture .blueprint {
          position: absolute;
          inset: -10%;
          opacity: 0.09;
          background-image:
            linear-gradient(rgba(125,211,252,0.9) 1px, transparent 1px),
            linear-gradient(90deg, rgba(125,211,252,0.9) 1px, transparent 1px);
          background-size: 64px 64px;
          animation: tunnelDrift 40s linear infinite;
          mask-image: radial-gradient(circle at 50% 42%, black 5%, transparent 68%);
        }
        .tunnel-aperture .ring {
          position: absolute;
          top: 42%;
          left: 50%;
        ...
        @media (prefers-reduced-motion: reduce) {
          .tunnel-aperture .ring,
          .tunnel-aperture .blueprint,
          .tunnel-aperture .mote { animation: none; opacity: 0.25; }
        }
      `}</style>
...
export default function Home() {
  ...
  // Subtle pointer-reactive glow — makes the hero feel alive without being gimmicky.
  ...
  return (
    <div className="min-h-screen bg-white font-vazir" dir="rtl">
      ...
      <section
        ...
        style={{
          // fallback values before the first pointermove event fires
          ['--mx' as any]: '50%',
          ['--my' as any]: '42%',
        }}
      >
        ...
        {/* pointer-reactive glow layer, sits above the tunnel field, below content */}
        <div
        ...
        {/* Stats strip, anchored to the base of the hero */}
        <div className="relative z-10 w-full px-4 sm:px-6 pb-10">
          <div className="max-w-4xl mx-auto grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
            {STATS.map(({ value, label, icon: Icon }) => (
              <div
                key={label}
                className="glass rounded-2xl px-4 py-4 text-center flex flex-col items-center gap-1.5"
              >
              ...
              return (
                <AnimatedSection key={feature.id} className={`delay-${(i + 1) * 200}`}>
                  <button
                    ...
                    className="group relative block w-full text-right rounded-3xl overflow-hidden h-full border border-navy-100 bg-white shadow-sm transition-all duration-300 hover:-translate-y-1.5 hover:shadow-2xl hover:shadow-navy-200/40"
                  >
                    ...
                    <div className="relative bg-white rounded-3xl h-full">
                      ...
                      <div className="p-7">
                        {/* Icon */}
                        <div className={`w-16 h-16 ${feature.iconBg} rounded-2xl flex items-center justify-center mb-6 shadow-sm transition-transform duration-300 group-hover:scale-110 group-hover:-rotate-3`}>
