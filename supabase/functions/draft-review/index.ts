...
۴. توجه بسیار مهم: خروجی شما نباید شبیه یک «گزارش تحلیل حقوقی جامع» (با فصل‌های متعدد، خلاصه مدیریتی مفصل، فهرست کامل قراردادهای بررسی‌شده، تحلیل ریسک عمومی و جمع‌بندی نهایی) باشد. خروجی شما فقط و فقط باید نتیجه مستقیم اجرای دستورالعمل «مقایسه و استخراج بندهای قراردادی» باشد: یعنی فهرست تمیز و مرتب بندهای پیشنهادی. از تولید هر نوع محتوای اضافه، حاشیه‌ای یا گزارش‌گونه که در دستورالعمل اصلی خواسته نشده خودداری کن.

ساختار html باید دقیقاً و فقط شامل این عناصر، به همین ترتیب باشد:

الف) یک h1 با عنوان دقیق: «بندهای پیشنهادی جهت تقویت پیش‌نویس قرارداد»

ب) بلافاصله بعد از h1، حداکثر یک پاراگراف کوتاه (p) که فقط تعداد قراردادهای مرجع بررسی‌شده و نام مختصر قرارداد پیش‌نویس را ذکر می‌کند؛ بدون هیچ تحلیل، جمع‌بندی یا نتیجه‌گیری اضافه در این پاراگراف.

ج) بلافاصله بعد از آن، یک جدول واحد (table) که تمام بندهای پیشنهادی طبق مرحله چهارم دستورالعمل اصلی در آن قرار دارند. اگر لازم بود بندها را برای خوانایی در گروه‌های موضوعی دسته‌بندی کنی، فقط مجاز هستی از h2 برای عنوان هر گروه (مثلاً «شروط تضمین و جبران خسارت») استفاده کنی و بلافاصله زیر آن گروه، جدول مربوط به همان گروه را بیاوری؛ اما هرگز h2 برای چیزی جز نام یک دسته‌بندی از بندهای پیشنهادی استفاده نکن.

د) اگر طبق مرحله چهارم دستورالعمل اصلی هیچ بند ارزش‌افزایی یافت نشد، به‌جای جدول فقط یک پاراگراف (p) بنویس که این موضوع را صریح و کوتاه اعلام می‌کند.

ه) هیچ بخش دیگری (مثل خلاصه مدیریتی، فهرست قراردادهای بررسی‌شده به‌صورت جداگانه، تحلیل ریسک کلی، یا جمع‌بندی و توصیه نهایی) تولید نکن. کل خروجی باید فقط شامل عنوان، یک پاراگراف معرفی کوتاه، و جدول(های) بندهای پیشنهادی باشد.

۵. اگر بندها را در چند جدول موضوعی گروه‌بندی کردی، هر جدول باید دقیقاً همان هفت ستون زیر و به همین ترتیب را داشته باشد. اگر گروه‌بندی نکردی، فقط یک جدول واحد با همین ستون‌ها کافی است: عنوان بند | علت پیشنهاد | منبع (قرارداد یا قراردادهای مرجع) | تحلیل حقوقی کوتاه | مزیت برای شرکت | ریسک ناشی از نبود آن | متن پیشنهادی جهت الحاق. محتوای هر سلول، به‌ویژه ستون «متن پیشنهادی جهت الحاق»، باید عبارت‌بندی کامل و آماده درج در قرارداد باشد، نه خلاصه یا اشاره کلی.
...
۸. اگر اطلاعات سند برای پاسخ‌گویی به بخشی از خروجی کافی نبود، این موضوع را صریحاً و در همان بخش (داخل html) اعلام کن؛ هرگز هیچ بند، قانون، ماده یا واقعیتی را جعل نکن.
`;
